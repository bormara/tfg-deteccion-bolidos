...
export class VotarComponent implements OnInit, AfterViewInit {
  ...
  getScreenSize(event?) {
    ...
    if (this.anchoPantalla > 3000) {
      console.log('Ancho pantalla mayor de 600px, numero de col');
      this.numeroDeColumnas = 2;
      this.altoDeFila = this.altoPantalla - 64;
      console.log('Tamaño fila:' + this.altoDeFila);
    } else {
      console.log('Ancho pantalla menor de 600px, numero de col');
  ...
  cambiarTamañoDiv() {
    ...
    if (this.anchoPantalla > 600) {
      ...
      altoFila = this.altoDeFila;
      izquierdaVideo = 0;
      // const altoVideo = this.altoDeFila / 2 - (this.anchoPantalla - 20) / (ratio * 2) + 56;
    }
